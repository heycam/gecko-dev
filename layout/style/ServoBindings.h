...
#define NS_DECL_THREADSAFE_FFI_REFCOUNTING_WITH_NAME(class_, name_)           \
...
#define NS_IMPL_THREADSAFE_FFI_REFCOUNTING_WITH_NAME(class_, name_)           \
  void Gecko_AddRef##name_##ArbitraryThread(class_* aPtr)                     \
  { NS_ADDREF(aPtr); }                                                        \
  void Gecko_Release##name_##ArbitraryThread(class_* aPtr)                    \
  { NS_RELEASE(aPtr); }
#define NS_DECL_THREADSAFE_FFI_REFCOUNTING(class_)                            \
  NS_DECL_THREADSAFE_FFI_REFCOUNTING_WITH_NAME(class_, class_)
#define NS_IMPL_THREADSAFE_FFI_REFCOUNTING(class_)                            \
  NS_IMPL_THREADSAFE_FFI_REFCOUNTING_WITH_NAME(class_, class_)

#define NS_DECL_HOLDER_FFI_REFCOUNTING_WITH_NAME(class_, name_)               \
...
#define NS_IMPL_HOLDER_FFI_REFCOUNTING_WITH_NAME(class_, name_)               \
  void Gecko_AddRef##name_##ArbitraryThread(ThreadSafe##name_##Holder* aPtr)  \
  { NS_ADDREF(aPtr); }                                                        \
  void Gecko_Release##name_##ArbitraryThread(ThreadSafe##name_##Holder* aPtr) \
  { NS_RELEASE(aPtr); }                                                       \
...
// Object refcounting.
NS_DECL_HOLDER_FFI_REFCOUNTING_WITH_NAME(RawGeckoPrincipal, Principal)
...
void Gecko_CopyBindingFrom(nsStyleDisplay* des, const nsStyleDisplay* src);
...
const char* Gecko_GetAttrAsUTF8(RawGeckoElement* element, const uint8_t* ns, uint32_t nslen,
                                const uint8_t* name, uint32_t namelen, uint32_t* length);
