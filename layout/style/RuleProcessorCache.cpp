    ...
    nsCSSRuleProcessor* aRuleProcessor)
{
  ...
  gRuleProcessorCache->DoPutRuleProcessor(aSheets, Move(aDocumentRules),
...
RuleProcessorCache::DoRemoveSheet(CSSStyleSheet* aSheet)
{
  ...
  for (auto it = last; it < mEntries.end(); it++) {
    for (DocumentEntry& de : it->mDocumentEntries) {
      de.mRuleProcessor->SetInRuleProcessorCache(false);
      mExpirationTracker.RemoveFromTracker(de.mRuleProcessor);
    }
  }
    ...
    nsCSSRuleProcessor* aRuleProcessor)
{
  ...
  if (!entry) {
    ...
    entry->mDocumentRules = aDocumentRules;
    for (CSSStyleSheet* sheet : aSheets) {
      sheet->SetInRuleProcessorCache();
    }
  } else {
    MOZ_ASSERT(entry->mDocumentRules == aDocumentRules,
...
void
RuleProcessorCache::ExpirationTracker::RemoveFromTracker(
    nsCSSRuleProcessor* aRuleProcessor)
{
