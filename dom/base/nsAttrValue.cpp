...
MiscContainer::Cache()
{
  ...
  switch (mType) {
    case nsAttrValue::eCSSDeclaration:
      ...
      break;
    case nsAttrValue::eServoDeclarationBlock:
      Servo_SetDeclarationBlockImmutable(mValue.mServoDeclarationBlock);
      break;
    default:
      MOZ_ASSERT_UNREACHABLE("unexpected cached nsAttrValue type");
      break;
...
void
nsAttrValue::SetToAlreadyAddrefed(ServoDeclarationBlock* aValue,
                                  const nsAString* aSerialized)
{
  MiscContainer* cont = EnsureEmptyMiscContainer();
  MOZ_ASSERT(cont->mValue.mRefCount == 0);
  cont->mValue.mServoDeclarationBlock = aValue;
  cont->mType = eServoDeclarationBlock;
  NS_ADDREF(cont);
  SetMiscAtomOrString(aSerialized);
  MOZ_ASSERT(cont->mValue.mRefCount == 1);
}
...
nsAttrValue::HashValue() const
{
  ...
  switch (cont->mType) {
    ...
    case eServoDeclarationBlock:
    {
      return NS_PTR_TO_INT32(cont->mValue.mServoDeclarationBlock);
    }
    default:
    {
...
nsAttrValue::Equals(const nsAttrValue& aOther) const
{
  ...
  switch (thisCont->mType) {
    ...
    case eServoDeclarationBlock:
    {
      return thisCont->mValue.mServoDeclarationBlock ==
               otherCont->mValue.mServoDeclarationBlock;
    }
    default:
    {
...
nsAttrValue::SetMiscAtomOrString(const nsAString* aValue)
{
  ...
  if (aValue) {
    uint32_t len = aValue->Length();
    // * We're allowing eCSSDeclaration and eServoDeclarationBlock attributes to
    //   store empty strings as it can be beneficial to store an empty style
    ...
    NS_ASSERTION(len || Type() == eCSSDeclaration ||
                 Type() == eServoDeclarationBlock || Type() == eEnum,
                 "Empty string?");
    MiscContainer* cont = GetMiscContainer();
    ...
    if (cont->IsRefCounted() && cont->mValue.mRefCount > 1) {
      ...
      switch (cont->mType) {
        ...
        case eServoDeclarationBlock:
        {
          ...
          cont->Release();
          cont->Evict();
          Servo_ReleaseDeclarationBlock(cont->mValue.mServoDeclarationBlock);
          break;
        }
        default:
        {
